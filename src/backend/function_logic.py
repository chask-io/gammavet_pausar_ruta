"""
Business logic for PausarRutaFn.

Org-specific Gammavet lambda: pause a driver route/session through the tenant
API and acknowledge the pause over WhatsApp.
"""
...
class FunctionBackend:
    def __init__(self, orchestration_event: OrchestrationEvent):
        ...
        logger.info(
            "PausarRutaFn initialized for org=%s",
    ...
    def _build_driver_payload(self) -> dict[str, Any]:
        ...
        if "driver_id" not in payload and "driver_phone" not in payload:
            raise ValueError("No se encontro driver_id ni driver_phone para pausar al conductor")
        return payload
    ...
    def _send_whatsapp(self, text: str) -> None:
        ...
        if not user_phone or not agent_phone:
            ...
            return
        self._dispatch_response_to_whatsapp(
            text,
            {"user_phone_number": user_phone, "agent_phone_number": agent_phone},
        )

    def _dispatch_response_to_whatsapp(self, text: str, extra_params: dict[str, Any]) -> None:
        ...
        whatsapp_event.event_id = evolved_uuid
        ...
        whatsapp_event.extra_params = evolve_response.get("extra_params", extra_params)
